...
# Compare a reference and an output. It is expected that the partial log generated by
# `regtests_ref_compare_impl` will be sent to stderr while the full log will be sent to stdout.
regtest_ref_compare() {
    local out_name=$1
...
# regtest <test-name> <command...>
# Define a test with name <test-name> that runs <command...>. The patterns {out.*}, {tmp.*}, and
# {ref} in the command are interpreted in a special way, as described in the manual.
regtest() {
...
# - _outputs: Space-separated list of (unique) output file names ({out.<ext>} patterns).
# - _tmpfiles: Space-separated list of (unique) temporary file names ({tmp.<ext>} patterns).
# - _extra_args: [--warn-only=<return-code>...]
#   If the <command...> returns any of the return codes specified by the `--warn-only` options,
#   its status will be recorded as "run(ignored)" and it will not be taken into account for
#   `regtest_print_summary`'s exit status (but will still result in a warning).
# The function that runs a regtest, handles its output, records its result, etc.
regtest_impl() {
    declare -A warn_only
    local arg
...
# Print a summary of all `regtest`s and return:
...
# regtest_start
# Initialise variables for the start of all tests.
regtest_start() {
...
# Run a full test suite test. For each suite <s> in <suites...>, run test suite file <dir>/<s>. If
# `regtest_run_suites_in_random_order` (is not empty), shuffle test suites. Take into account
# `regtest_suite_timeout` variable and "regtest-timeout-factor" comments.
regtest_run_suites() {
    local dir=$1
    shift
...
# Run a single function as a full test suite set.
